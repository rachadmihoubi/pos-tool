...
Component 3 (auto-update)'s elevation gap and the update-loop marker gap
are both fixed and the elevation mechanism itself is real-machine-verified
(2026-08-27, see the component table above) — the only thing left before
`update.enabled` should be flipped on for a real store is a real
frozen-build silent install end-to-end (needs Inno Setup installed
somewhere first). Read the component table and
`docs/superpowers/specs/2026-08-27-update-elevation-fix.md` before touching
`poslib/updater.py`, `packaging/setup.iss`, or `main.py`'s `--apply-update`
dispatch. Next build-order step: that end-to-end frozen-build verification,
and Component 5 (hub + cross-store stock search), which has no plan written
yet.
