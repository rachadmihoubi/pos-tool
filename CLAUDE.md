...
| 3 | Silent auto-update via GitHub Releases | **CODE-COMPLETE; NOT customer-rollout-ready** | `docs/superpowers/plans/2026-08-26-component3-auto-update.md` (the plan's own SDD ledger has been deleted per the finishing-a-development-branch process now that final review is clean — this row, commit history, and the `project_component3_elevation_gap` memory are the durable record). Detect/download/checksum-verify/reject all verified 2026-08-27 against a real, now-deleted throwaway `v1.0.1` GitHub release. **Known blocking gap #1, found only by the final whole-branch review, not fixed yet**: the watcher's scheduled task runs de-elevated (`setup.iss:33`, intentionally - see commit `10deb48` for why), but the installer it must silently relaunch for an update has no `PrivilegesRequired` override and installs to Program Files, which a de-elevated process cannot write to - a real customer PC would either fail to update or hit an unattended UAC prompt. The two cheap fixes (per-user install; a `/CURRENTUSER` override on just the update run) were both investigated and ruled out - they either break Component 2's already-verified scheduled-task creation or install a second, parallel copy. The real fix (a second always-elevated on-demand helper scheduled task) is new design surface, not a one-line patch - not built. **Do not ship Component 3 to a real store PC until this is resolved.** `config.template.yaml`'s `update.enabled` now defaults to `false` (with a comment pointing here), and — after a gap in that mitigation was caught 2026-08-27 (a config.yaml with no `update:` section, which is what a fresh install produces if the template flip doesn't reach it, would have fallen through to `poslib/updater.py`'s own `cfg.get("update.enabled", True)` fail-*live* default) — that code default was also flipped to `False`, so the "off unless explicitly turned on" ruling now holds unconditionally, not only when the template happens to be the config's source. **Known gap #2, narrower, also not fixed**: no persisted marker records the last version an update attempt tried, so a genuinely mis-cut release (bundled `VERSION` not actually newer) could loop download→install→relaunch indefinitely; the fix is a small marker file under `user_data_dir()` checked before `_write_version()`/`_build()` run — not built. Two more items genuinely need a human at the keyboard, not a code change: watching a real silent install for a UAC prompt/dialog, and exercising the "already up to date" no-op path against a real frozen build. |
...
Component 3 (auto-update) is code-complete but has a known elevation gap
blocking real customer rollout - see the component table above before
touching `poslib/updater.py`, `packaging/setup.iss`, or `watcher.py`'s
update-check call. Next build-order step: resolve that gap (needs a real
design decision, not a quick patch) and Component 5 (hub + cross-store
stock search), which has no plan written yet.
