...
| 3 | Silent auto-update via GitHub Releases | **CODE-COMPLETE; NOT customer-rollout-ready** | `docs/superpowers/plans/2026-08-26-component3-auto-update.md`, ledger at `.superpowers/sdd/2026-08-26-component3-auto-update/progress.md`. Detect/download/checksum-verify/reject all verified 2026-08-27 against a real, now-deleted throwaway `v1.0.1` GitHub release. **Known blocking gap, found only by the final whole-branch review, not fixed yet**: the watcher's scheduled task runs de-elevated (`setup.iss:33`, intentionally - see commit `10deb48` for why), but the installer it must silently relaunch for an update has no `PrivilegesRequired` override and installs to Program Files, which a de-elevated process cannot write to - a real customer PC would either fail to update or hit an unattended UAC prompt. The two cheap fixes (per-user install; a `/CURRENTUSER` override on just the update run) were both investigated and ruled out - they either break Component 2's already-verified scheduled-task creation or install a second, parallel copy. The real fix (a second always-elevated on-demand helper scheduled task) is new design surface, not a one-line patch - not built. **Do not ship Component 3 to a real store PC until this is resolved.** `config.template.yaml`'s `update.enabled` now defaults to `false` (with a comment pointing here) so this ruling is enforced by what a fresh customer install actually ships with, not only by this doc. Two more items genuinely need a human at the keyboard, not a code change: watching a real silent install for a UAC prompt/dialog, and exercising the "already up to date" no-op path against a real frozen build. |
