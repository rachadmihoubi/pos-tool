...
Component 3 (auto-update)'s elevation gap is fixed (2026-08-27, see the
component table above) but still needs real-machine verification (the new
SYSTEM scheduled task actually being created and firing with no UAC prompt,
a real silent install, the no-op path) before `update.enabled` should be
flipped on for a real store — read the component table and
...
dispatch. Next build-order step: that real-machine verification, Known gap
#2 (update-loop marker file), and Component 5 (hub + cross-store stock
search), which has no plan written yet.
