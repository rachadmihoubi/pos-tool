...
**Paused here, deliberately — resume by asking the user for go-ahead
first, not by just proceeding:**
- **Task 6 Step 4** (manual smoke test: build a real `Setup.exe` via
  `ISCC.exe packaging\setup.iss`, run it against a disposable throwaway
  Cloudflare project/token, confirm the wizard page and provisioning flow
  work end-to-end) and **Task 7** (live disposable-project full
  provisioning run + idempotency re-run + teardown) are explicitly
  combinable — do both in one pass. **Task 8** (docs) comes after.
- The plan's own Global Constraints gate both Task 6 (this live-run part)
  and Task 7 on the user's explicit go-ahead — same as Task 1 was gated
  earlier in this same plan. That go-ahead was asked for on 2026-08-29 and
  the user chose **"stop here and commit everything to continue working on
  the store PC"** rather than continue on this dev machine — not a
  rejection of the plan, a change of *where* to run the live-account step.
  The branch was pushed to origin specifically so a session on the store
  PC can pull it and pick up exactly here.
- When resuming (on the store PC or elsewhere): re-read the ledger file
  above, confirm you're on `worktree-component5-cloudflare-provisioning`
  (or a fresh checkout of it), and ask for go-ahead + a disposable
  Cloudflare token before doing Task 6 Step 4 / Task 7 — do not assume a
  prior go-ahead carries over to a new machine/session.
