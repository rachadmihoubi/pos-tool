...
| 5 | Multi-store hub page + cross-store stock search | **DESIGN SETTLED 2026-08-27, NOT BUILT** | `docs/superpowers/specs/2026-08-27-component5-hub-design.md` — corrects a real gap found in the original master spec (a Cloudflare Pages deployment fully replaces a project's site, so 3 stores can't share one hub project as a write target the way the master spec assumed) and a hard blocker (Access login sessions can't cross `*.pages.dev` origins — cookie scoping, not a Cloudflare quirk). Settled design: each store keeps pushing only its own project; one path, `/stock.json`, gets an Access **Bypass** policy so it's reachable without login (name + quantity + price — price inclusion was the owner's explicit call, a real widening from "low-sensitivity" but accepted); the hub stays a plain static page whose JS fetches all 3 `stock.json` files directly, no new Cloudflare product. Owner's explicit goal: **the installer should provision all of this automatically**, no manual Zero Trust dashboard clicking per store — resolved as a one-time-use powerful Cloudflare token, pasted in only during a *new* store's interactive install (never persisted), that creates the project + Access app + both policies via Cloudflare's Access Management API and then mints the narrow ongoing `Pages:Edit`-only token for the watcher's permanent use — preserves Component 4's least-privilege intent for the credential that actually lives on the till PC long-term. The Access Management API itself is unverified from this codebase (distinct from the already-reverse-engineered Pages Direct Upload API) and needs its own research-and-test pass against a disposable project before trusting it unattended. No auto-matching across stores, no combined totals — unchanged from the master spec, V1 shows matching rows side by side. |
